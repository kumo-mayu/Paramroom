// Where packets go. Kept behind interfaces so the app can be exercised without VRChat (IMAGEPAD_TARGET) and tests can
...
public sealed class FixedTargetFinder(VrcClient client) : ITargetFinder
{
    public static FixedTargetFinder? FromSpec(string? spec)
    {
        ...
        var p = spec.Split(':');
        if (p.Length < 2 || !int.TryParse(p[1], out int port)) throw new ArgumentException($"IMAGEPAD_TARGET の形式が違います: {spec}（host:port[:Int の数[:format]]）");
