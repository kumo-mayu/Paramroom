...
public sealed class ImagePadSession : IAsyncDisposable
{
    ...
    public void SelectTarget(string name)
    {
        ...
        if (!IsUsable(t)) throw new ImageSourceException("そのクライアントのアバターには ImagePad が入っていないようです。ImagePad を入れたアバターに着替えてから探し直してください。");
    ...
    public async Task StartSendingAsync()
    {
        ...
        if (s0.Target is not { } target)
            throw new ImageSourceException(s0.Targets.Count == 0
                ? "送信先の VRChat が見つかっていません。VRChat を起動し（OSC を有効にして）ImagePad 入りのアバターを着てから「送信先を探す」を押してください。"
                : "送信先を選んでください。");
