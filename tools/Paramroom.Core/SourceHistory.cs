// Recently used image sources (files and URLs), newest first. Kept as readable JSON (%LOCALAPPDATA%\Paramroom\history.json,
// PARAMROOM_HOME overrides the folder) so it can be looked at or edited without the app.
...
public sealed class JsonFileHistory(string path) : ISourceHistory
{
    ...
    {
        var home = Environment.GetEnvironmentVariable("PARAMROOM_HOME");
        if (string.IsNullOrWhiteSpace(home)) home = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Paramroom");
