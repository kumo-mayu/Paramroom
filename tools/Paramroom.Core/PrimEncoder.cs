...
public sealed unsafe class PrimEncoder
{
    ...
    public PrimResult Encode(int P, Action<int, int>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        if (Environment.GetEnvironmentVariable("PARAMROOM_PROFILE") == "1") Console.Error.WriteLine($"profile: rand {prof[0]:F2} s, climbs {prof[1]:F2} s, polish {prof[2]:F2} s, apply {prof[3]:F2} s");
