...
// Paramroom_Format (local-only avatar parameter set by the prefab) -> decoder variant
...
// Must match the Unity prefab builder (ParamroomBuilder.Formats).
public sealed record DecoderFormatInfo(int R, int N, int Cb, int Rb, int Ab, int[] Col, int ABits, string Prefab, string Name)
{
    ...
    // those that still leave 8 spare bits for the aspect code. Same rule as ParamroomBuilder.GoodLayouts, so an
...
public static class ParamNames
{
    ...
    // the old prefabs called it Paramroom_Format; a prefixed one uses Paramroom_Format
    public static string Format(string prefix) => prefix.Length > 0 ? prefix + "Format" : "Paramroom_Format";
}
    ...
    {
        {
            ...
            try
            {
                ...
                if (Environment.GetEnvironmentVariable("PARAMROOM_DEBUG") == "1")
