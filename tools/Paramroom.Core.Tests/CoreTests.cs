...
{
    // the table the Unity builder offers (measure/unity/.../ParamroomBuilder.cs GoodLayouts, sim/codecs/prim.js layout)
    [Theory]
...
public class SessionTests
{
    ...
    public async Task EncodesForTheFoundAvatarAndSendsItsPacketSize()
    {
        ...
        await using var session = new ParamroomSession(new FakeFinder(client), _ => transport, new StbImageDecoder(), new HttpImageFetcher(),
    ...
    {
        await using var session = new ParamroomSession(new FakeFinder(), _ => new CaptureTransport(), new StbImageDecoder(), new HttpImageFetcher());
...
public class HoldTests
{
    ...
    {
        await using var session = new ParamroomSession(new NoFinder(), _ => throw new InvalidOperationException(), new StbImageDecoder(), new HttpImageFetcher());
        var handler = new CommandHandler(session);
...
{
    // must match ParamroomBuilder.Formats / GoodLayouts in the Unity project
    [Theory]
    ...
    {
        var spec = Paramroom.Session.DecoderSpec.For(new VrcClient("c", "127.0.0.1", 9000, null, 32, 99, null));
    ...
    public void SpecFollowsTheAvatarFormat()
    {
        ...
        var spec = Paramroom.Session.DecoderSpec.For(client);
        Assert.Equal(8, spec.Format.Cb);
        Assert.False(spec.SameLayout(Paramroom.Session.DecoderSpec.For(client with { Format = 3 })));
    }
}
...
public class PrimCountTests
{
    ...
    public async Task NoticesWhenTheTargetDisappears()
    {
        ...
        await using var session = new ParamroomSession(finder, _ => throw new InvalidOperationException(), new StbImageDecoder(), new HttpImageFetcher(),
    ...
    // a fixed address (PARAMROOM_TARGET) cannot be asked over OSCQuery, so the scan loop must leave it alone
    ...
    {
        await using var session = new ParamroomSession(new FixedClient(), _ => throw new InvalidOperationException(), new StbImageDecoder(), new HttpImageFetcher(),
            options: new SessionOptions { TargetScanInterval = TimeSpan.FromMilliseconds(30) });
        await session.RefreshTargetsAsync();
        Assert.NotNull(session.Snapshot.Target);
...
public class DropAndHistoryTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "paramroom-test-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            await using (var session = new ParamroomSession(new FixedTargetFinder(new VrcClient("c", "127.0.0.1", 1, null, 32, 3, null)), _ => throw new InvalidOperationException(), new StbImageDecoder(), new HttpImageFetcher(), new SessionOptions { EncodePrimsLimit = 50 }, store))
            {
            ...
            await using var reopened = new ParamroomSession(new FixedTargetFinder(new VrcClient("c", "127.0.0.1", 1, null, 32, 3, null)), _ => throw new InvalidOperationException(), new StbImageDecoder(), new HttpImageFetcher(), history: store);
