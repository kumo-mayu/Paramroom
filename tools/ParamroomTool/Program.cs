// paramroom: prim encoder + OSC sender for the Paramroom avatar decoder (C# port of measure/osc/send-image.js).
//
//   paramroom send   <image> [--fit stretch|crop] [--epoch 1] [--schedule fast+sqrt/8(default)|sqrt|fast|fast+sqrtB/k|fast+baseB/k|carousel] [--hold 100] [--duration 0]
//                           [--client <name part | OSC port>] [--wait 3] [--R 512 --n 4000] [--force] [--seed N]
//                           [--host 127.0.0.1 --port 9000] [--no-bundle]
//   paramroom send   --units units.json [...]                  (send a previously encoded image)
//   paramroom encode <image> [--R 512 --n 4000 | --format 3..6] [--fit ...] [--refine 3] [--out units.json] [--png canvas.png]
//   paramroom list                                             (VRChat clients found with OSCQuery)
//
...
// Decoder variant: the prefab's local-only parameter Paramroom_Format (1 = 256/1000, 2 = 512/2000, 3 = 512/4000,
...
{
    Console.WriteLine("usage: paramroom send|encode|list [<image>] [--units units.json] [--fit stretch|crop] [--epoch 1] [--schedule fast+sqrt/8(default)|sqrt|fast|fast+sqrtB/k|fast+baseB/k|carousel] [--client name|port] [--R 512 --n 4000] [--out units.json] [--png out.png]");
    return 1;
}
string cmd = argv[0];
...
if (cmd == "send")
{
    ...
    else
    {
        ...
        var sel = all.Where(c => c.ParamroomParams > 0 && c.Problem == null).ToList();
        if (Opt("client") is string want) sel = all.Where(c => c.Name.Contains(want) || c.OscPort.ToString() == want).ToList();
        if (sel.Count != 1)
        {
            Console.WriteLine(sel.Count == 0 ? "no VRChat client with the Paramroom Int parameters D0.. found (use --client or --port)" : "several VRChat clients match: choose one with --client <name part | OSC port>");
            return 2;
        }
        host = sel[0].OscIp; port = sel[0].OscPort; format = sel[0].Format; avatarBytes = sel[0].ParamroomParams; paramPrefix = sel[0].ParamPrefix;
...
if (format is int f)
{
    if (!DecoderFormat.Known.TryGetValue(f, out var kf)) { Console.WriteLine($"unknown Paramroom_Format {f}"); return 2; }
    avatarFormat = kf;
