...
// viewer present from the start (measured channel model meas60, one seed).
// usage: node fastfirst-visual.js <outDir> [image ...]
const fs = require('fs');
...
const TIMES = [5, 10, 20, 40, 60];
const outDir = path.resolve(process.argv[2]);
const images = process.argv.slice(3).length ? process.argv.slice(3) : ['screenshot_mahara', 'kodim23'];
...
for (const image of images) {
  ...
  scheds.forEach(([name, sched], si) => {
    const arr = T.arrivals({ schedule: sched, H: 0.1, channel: { sampler: 'meas60', H: 0.1, torn: 0 }, join: 0, tMax: 60, seed: 7919 + 13, packetBytes: 32 });
    ...
    for (const t of TIMES) {
      ...
      const r = dec.render();
      const p = path.join(tmp, `${image}-${si}-${t}.png`);
      I.savePNG(I.resize(r, 384, 384), p);
      files.push(p);
      labels.push(`${name}　${t} 秒（${got.size}/${N} ユニット、${M.all(ref, r).msssimc.toFixed(3)}）`);
    }
  });
  ...
  args.push('-filter_complex', filter, '-map', '[out]', '-frames:v', '1', path.join(outDir, `${image}-sqrt-vs-fast.png`));
