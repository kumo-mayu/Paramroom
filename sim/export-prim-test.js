...
// usage: node export-prim-test.js [image=kodim23] [out=../measure/unity/Assets/ImagePadMeasure/TestData] [--stretch] [--R 512 --n 4000]
...
const argv = process.argv.slice(2).filter((a, i, all) => !a.startsWith('--') && !(i > 0 && /^--(R|n)$/.test(all[i - 1])));
...
const name = `prim-${image}${stretch ? '-stretch' : ''}${R === 256 && nPrims === 1000 ? '' : `-r${R}-n${nPrims}`}`;
