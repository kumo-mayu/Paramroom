'use strict';
// PRIM: "primitive shapes" codec (in the spirit of fogleman/primitive).
//
...
function geomWidths(cfg) {
  ...
  if (cfg.shape === 'ell') return [cfg.cb, cfg.cb, cfg.rb, cfg.rb, cfg.ab];
...
function makeGeom(cfg) {
  ...
  function geom(codes, g) {
    if (shape === 'ell') {
      ...
      g[3] = Math.max(0.5, R / 2 * ((codes[3] + 1) / rN) ** 2);
      g[4] = codes[4] / aN * Math.PI;
    } else {
      for (let i = 0; i < codes.length; i++) g[i] = codes[i] / cmax * R;
    }
  }
  // fills idx with covered pixel indices, returns count
  function raster(g, idx, st = 1) {
    let n = 0;
    if (shape === 'tri') {
      ...
      for (let y = by0 + (st - by0 % st) % st; y <= by1; y += st) {
        ...
        for (let x = bx0 + (st - bx0 % st) % st; x <= bx1; x += st) {
          ...
          if (u * u * irx + v * v * iry <= 1) idx[n++] = row + x;
        }
      }
    } else {
...
function blend(cur, idx, n, rgb, a) {
  const ia = 1 - a, r = rgb[0] * a, gg = rgb[1] * a, b = rgb[2] * a;
  for (let i = 0; i < n; i++) {
    const o = idx[i] * 3;
    cur[o] = cur[o] * ia + r; cur[o + 1] = cur[o + 1] * ia + gg; cur[o + 2] = cur[o + 2] * ia + b;
  }
}
...
function cfgOf(o) {
  ...
  const label = `${geo}-c${col.join('')}a${o.aBits || 0}-r${o.R}-n${o.maxPrims}`;
...
module.exports = {
  ...
  encode(ref, cfg, P, base) {
    ...
    function evaluate(codes, out, st = 1) {
      G.geom(codes, g);
      let n = G.raster(g, idx, st);
      if (st > 1 && n < 64) { st = 1; n = G.raster(g, idx, 1); }
      if (n === 0) { out[0] = 0; out[1] = out[2] = out[3] = 0; return 0; }
      return scoreSums(n, out) * st * st;
    }
    function scoreSums(n, out) {
      ...
      for (let q = 0; q < nAlpha; q++) {
        ...
        const cc = [0, 0, 0];
        for (let ch = 0; ch < 3; ch++) {
          const b = ch * 5, Sd = S[b], Sdc = S[b + 1], Sc = S[b + 2], Scc = S[b + 3], Sdd = S[b + 4];
          let col = Sc / n + Sd / (a * n);
    ...
    function randomShape(codes) {
      ...
      if (cfg.shape === 'tri') {
        ...
        codes[0] = toCode(px, 0); codes[1] = toCode(py, 1);
        codes[2] = radCode(sz * (0.2 + 0.8 * rnd())); codes[3] = radCode(sz * (0.2 + 0.8 * rnd()));
        codes[4] = Math.floor(rnd() * (gmax[4] + 1));
      } else {
    ...
    function mutate(src, dst) {
      ...
      for (let t = 0; t < nf; t++) {
        ...
        if (cfg.shape === 'ell' && f === 4) dst[f] = (dst[f] + d + m + 1) % (m + 1); // angle wraps
    ...
    for (let j = 0; j < target; j++) {
      ...
      blend(cur, idx, n, [colVal(best.r[1], crb), colVal(best.r[2], cgb), colVal(best.r[3], cbb)], alphas[best.r[0]]);
      prims.push(best);
    ...
    return {
      ...
      render(base) {
        ...
        for (let j = 0; j < L.maxPrims; j++) {
          ...
          const rgb = cfg.col.map(w => colVal(rd.read(w), w));
