...
// Used by the decoder prefab builder (ParamroomBuilder) and the measurement prefab builder (ParamroomMeasure).
using System;
using System.Linq;
using UnityEditor;
using UnityEngine;
...
public static class ParamroomModularAvatar
{
    ...
    static SerializedProperty Req(SerializedObject so, string name)
    {
        ...
        if (p == null) throw new Exception($"[Paramroom] property '{name}' not found on {so.targetObject.GetType().FullName}");
        return p;
    }
    static SerializedProperty Req(SerializedProperty sp, string name)
    {
        var p = sp.FindPropertyRelative(name);
        if (p == null) throw new Exception($"[Paramroom] property '{name}' not found in {sp.propertyPath}");
        return p;
    }
    ...
    internal static void AddMergeAnimator(GameObject root, RuntimeAnimatorController ctrl)
    {
        ...
        if (t == null) { Debug.LogWarning("[Paramroom] Modular Avatar not found: add the FX controller to your avatar manually."); return; }
    ...
    // D0..D(n-1) synced Int; formatId > 0 also adds Paramroom_Format, a local-only (not synced, 0 sync bits) Int whose
    ...
    internal static void AddParameters(GameObject root, int byteCount, int formatId = 0)
    {
        ...
        if (t == null) { Debug.LogWarning("[Paramroom] Modular Avatar not found: add D0..D(n-1) Int synced parameters manually."); return; }
        var c = root.AddComponent(t);
        var so = new SerializedObject(c);
