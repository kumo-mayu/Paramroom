// Paramroom local image - one group of 32 Float parameters (camera A of the store loop only). Same transport as the
// probe (measure/unity/Assets/ParamroomLocalProbe, docs/research/12); the store holds 4096 chunks and has no test pattern.
//
...
Shader "Paramroom/LocalImageGroup"
{
    Properties
    {
        ...
        _AtlasH ("Atlas height", Float) = 516
        _StoreRows ("Store rows", Float) = 512
        _F0 ("F0", Float) = 0
        _F1 ("F1", Float) = 0
        _F2 ("F2", Float) = 0
        _F3 ("F3", Float) = 0
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                if (quad == 0)
                {
                    ...
                    o.info = uint4(0, (uint)x0, 0, 0x80000000u | chunk);
                }
                else
                {
                    float g = _Group;
