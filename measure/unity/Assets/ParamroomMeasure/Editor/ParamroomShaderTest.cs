// Paramroom prim decoder shader test (batchmode with graphics):
//   Unity -batchmode -projectPath <p> -executeMethod ParamroomShaderTest.Run -paramroomOut <dir> [-paramroomData <TestData json name>]
...
public static class ParamroomShaderTest
{
    ...
    public static void Run()
    {
        ...
        try
        {
            var json = System.IO.File.ReadAllText($"Assets/ParamroomMeasure/TestData/{dataName}.json");
            ...
            // store size as ParamroomBuilder.StoreTexels (whole rows, at least 8192 texels)
            ...
            expTex.LoadImage(System.IO.File.ReadAllBytes($"Assets/ParamroomMeasure/TestData/{dataName}-expected.png"));
            ...
            var p2 = ReadAtlas();
            Debug.Log($"[Paramroom] prim idle: 120 idle steps {sw3.ElapsedMilliseconds} ms ({sw3.ElapsedMilliseconds / 120.0:F2} ms/step) vs drawing {(double)sw2.ElapsedMilliseconds / (nBatches * 2 + 4):F2} ms/step; after repeated packet counter={Ctrl(p2, 0).r} dirty={Ctrl(p2, 6).r} (expect 0/0)");
            ...
            var p4 = ReadAtlas();
            var cmp4 = Compare(p4);
            Debug.Log($"[Paramroom] prim split feed: after half counter={Ctrl(p3, 0).r} dirty={Ctrl(p3, 6).r}; after all maxdiff={cmp4.maxDiff} counter={Ctrl(p4, 0).r} dirty={Ctrl(p4, 6).r} epoch={Ctrl(p4, 1).r}");
            ...
            Debug.Log($"[Paramroom] prim view: aspect accepted={Ctrl(p4, 4).r}, quad {vw}x{vh} px (w/h {(float)vw / vh:F3}); present prims={present}/{NPrims}");
